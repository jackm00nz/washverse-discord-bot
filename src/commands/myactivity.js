...
export default {
  data: new SlashCommandBuilder().setName("myactivity").setDescription("View your activity information"),
  ...
  async execute(interaction) {
    ...
    try {
      ...
      let activity = dbUtils.getActivity(interaction.user.id)
      ...
      if (dbUser && dbUser.roblox_id) {
        ...
        // You would need to map rank numbers to names based on your group structure
        rankName = `Rank ${rank}`
      ...
      const embed = new EmbedBuilder()
        .setTitle(`📊 Activity Report for ${interaction.user.username}`)
      ...
      if (overallCompletion >= 100) {
        embed.setFooter({ text: "✅ You are meeting all activity requirements!" })
      } else {
        embed.setFooter({ text: "⚠️ You are not meeting all activity requirements." })
      }
